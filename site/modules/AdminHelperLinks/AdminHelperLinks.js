function getPageTemplateName() {
  ...
  var templateClass = $pageEditProcess
    .attr("class")
    .split(/\s+/)
    .find((c) => c.startsWith("template_"));
...
function initField() {
  ...
  if (fieldId) {
    ...
    $hoverContent.append(
      "<span class='ahl__field-name' title='Field name'>" +
        fieldName +
        "</span>"
    );
    $hoverContent
    ...
    $editLink.attr(
      "href",
      ProcessWire.config.urls.admin + "setup/field/edit?id=" + fieldId
    );
    $editLink.attr(
      "class",
      "ahl__link ahl__link--edit-field pw-modal pw-modal-medium"
    );
    ...
    var templateFieldgroupId =
      ProcessWire.config.AdminHelperLinks.templateFieldgroupIds[templateName];

    if (templateFieldgroupId) {
      var $editInContextLink = $("<a>");
      $editInContextLink.attr(
        "href",
        ProcessWire.config.urls.admin +
          "setup/field/edit?id=" +
          fieldId +
          "&fieldgroup_id=" +
      ...
      $editInContextLink.attr(
        "class",
        "ahl__link ahl__link--edit-field-in-context pw-modal pw-modal-medium"
      );
...
function initPageTemplate() {
  ...
  var templateId =
    ProcessWire.config.AdminHelperLinks.templateIds[templateName];

  if (templateId) {
    var $wrapper = $('<li class="ahl__template" title="Edit template">');

    $wrapper.append(
      '<span class="ahl__template-name" title="Template">' +
        templateName +
        "</span>"
    );
    ...
    $link.attr(
      "href",
      ProcessWire.config.urls.admin + "setup/template/edit?id=" + templateId
    );
    $link.attr(
      "class",
      "ahl__link ahl__link--template pw-modal pw-modal-medium"
    );
